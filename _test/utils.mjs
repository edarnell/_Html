...
const debug = console.log.bind(console),
    ...
    dir = path.join(__dirname, '..', '..', 'test', 'html'),
    nodeDir = path.join(__dirname, '..', 'node', 'test'),
    confDir = path.join(__dirname, '..', 'server')
...
function waitForFile(fn, i = 100, t = 1000) {
    if (fn.endsWith('.css')) fn = path.join(dir, fn)
...
export { setPage, sleep, setDebug, debug, token, scp, uc1, hover, tt, rm, mv, waitForFile, url, logout, login, name, waitSel }